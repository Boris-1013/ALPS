...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    ...
    with torch.no_grad():
        ...
        for i, (batch_x, batch_y) in tqdm(enumerate(vali_loader)):
            #batch_x = batch_x.squeeze(-1).float()#alibaba2021专用，因为他有三维
            #batch_y = batch_y.squeeze(-1).float()#alibaba2021专用，因为他有三维
            ...
            pred = outputs.detach()#与梯度无关
            true = batch_y.detach()#与梯度无关
    ...
    rmse_loss = np.sqrt(total_loss)
    total_mae_loss = np.average(total_mae_loss)

    model.train()
    return rmse_loss, total_mae_loss
...
def vali_single(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    total_loss = []
    total_mae_loss = []
    model.eval()
    with torch.no_grad():
